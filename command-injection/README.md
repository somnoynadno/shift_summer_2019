...
## Описание
Command injection – это атака, основанная на внедрении вредоносного кода в уязвимом приложении. Атаке подвержены сервисы не фильтрующие пользовательский ввод и исполняющие команды операционной системы с помощью функций exec(), system() и им подобных.

Причиной возникновения является то, что bash/powershell выполняет не одну команду, а полностью интерпретирует строку, поданную на вход. Суть атаки состоит в том, чтобы путём модификации ввода на выполнение пошла строка вида "легитимная_команда <разделитель> вредоносная_команда", где добавляются разделители вида ';', '&&' и прочие, в т.ч. кавычки и нуль-терминатор.
...
#### Blind инъекция
Пример кода, исполняемого в отдельном потоке:
```
shell_exec('curl -vvv -L -sD - -o /dev/null '.$target.' > /dev/null &' );
```
В данном случае символ & в конце выражения говорит о том, чтобы ОС запустила процесс в фоновом режиме. Но данный код уязвим для слепой bash-инъекции.

Из-за отдельного потока мы не сможем проследить выполнение команды sleep. Однако в данном случае мы можем только создать файл и проверить его присутсвие, применив такой зловред:
```
echo "<echo "pwned">" > test.php
```
А затем пробуем обратиться к созданной странице через браузер. Если страница существует, то сервер уязвим для инъекций.
...
Скачать утилиту: [kali.tools](https://kali.tools/?p=1107)

- Burp Suite – великолепный инструмент для тестирования на проникновение и автоматизации эксплуатирования уязвимостей.

Ссылка на скачивание: https://portswigger.net/burp/communitydownload.

- Также для поиска уязвимостей вам могут помочь типичные инструменты пентестера, такие как сканер портов (например, nmap), брутер директорий (dirbuster) и tcp-утилита netcat.
...
- Фильтровать пользовательский ввод средствами языка (по подстрокам и регулярным выражениям). Потенциально опасные символы – любые разделители shell-команд (';', '&&', '||', '&', '|'), апостроф и кавычки.
...
Яркий пример плохого фильтра:
```
if (isset($_GET['curl']) && !empty($_GET['curl']) &&!preg_match('/\s+/',$_GET['curl'])){
	curl($_GET['curl']);
}
```
Возможно, разработчик просто скопировал регулярку, которая обещала быть безопасной, но сам сайт не тестировал, потому что через это регулярное выражение спокойно проходят команды с разделителями и можно спокойно эксплуатировать уязвимость.
...
- https://helpugroup.ru/kak-ispolzovat-komandnuyu-inektsiyu-chtoby-podnyat-reverse-shell-na-veb-servere/ - ещё об эксплуатации
- https://cryptoworld.su/zametaem-sledy-posle-vzloma/ - стираем следы взлома
