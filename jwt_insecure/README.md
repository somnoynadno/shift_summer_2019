...
Это означает, что когда пользователь отправляет токен для авторизации, в бэкэнде не выполняется проверка подписи. Все библиотеки допускают использование опции «none» для алгоритма подписи. Base 64 кодирует данные перед первой точкой, включая информацию об алгоритме.  Можно заменить алгоритм на «none», и некоторые реализации будут принимать это как законный токен,

Таким образом, злоумышленник может изменить алгоритм на «none», удалить подпись и использовать измененный токен для доступа к серверу.
